...
4. Sends email reminder to the dog owner (tutor), using the "Nombre de pila" column for the greeting
...
7. On any fatal error, sends an alert email to the manager (no silent failures)
...
- **Configuration:** All tunable values live in the `CONFIG` block at the top of the script. They can be overridden per-deployment via **Script properties** (Project Settings → Script properties) without editing code: `EMAIL_RESUMEN`, `LOGO_DRIVE_ID`, `SPREADSHEET_ID`, `HOJA_CLIENTES`, `HOJA_LOG`, `TZ`.
- **Sheet schema — Clientes:** `A Perro/a | B Tutor/a | C Nombre de pila | D Email | E Teléfono | F Notas`. Column C (Nombre de pila) is optional but recommended for compound Spanish names (María José, Juan Carlos…). If empty, the script falls back to the first word of column B.
